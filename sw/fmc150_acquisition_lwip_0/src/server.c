...
int get_command_handler(unsigned int comm, struct command_handler *comm_handler){
	...
	switch(comm){
		...
		case LED_WRITE:
			...
			break;
			/* Dummy sctructure for read/write soft_regs */
		case SOFT_REG_READ:
		case SOFT_REG_WRITE:
			comm_handler->comm_attr = &soft_reg_attr;
			comm_handler->comm_ops = &soft_reg_ops;
			break;
		default:
			error = ERROR;
	}
...
int get_response_handler(unsigned int comm, struct response_handler *res_handler){
	...
	switch(comm){
		...
		case SOFT_REG_READ:
		case SOFT_REG_WRITE:
			res_handler->new_data_id = SOFT_REG_NEW_DATA;
			res_handler->remaining_data_id = SOFT_REG_REMAINING_DATA;
			res_handler->byte_offset = 0;
			break;
		default:
			error = ERROR;
...
int handle_request(struct command_packet *command_packet_r, struct response_packet *response_packet_s){
	...
	switch(comm_handler.comm){
		...
		case DDC_FMC150_GET_SAMPLES:
			...
			//dump_mem_adc(((unsigned char *)(dma_buf/*low_lev_handler->attr->mem_start_addr*/) + res_handler.byte_offset),
		...
		case SOFT_REG_READ:
			if(comm_handler.comm_ops->read_reg(cs, addr, &val) < 0){
				...
				response_packet_s->comm = res_handler.new_data_id;
